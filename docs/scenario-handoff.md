...
page) transposes exactly and is deliberately not built until an install asks
for it: a config is already JSON, and a JSON document sits in it natively —
which prose did not.
...
- **The loaded model wins.** A file written for CI points
  ...
  model in front of the reader, and the disagreement is flagged — the
  `arazzo-source-ambiguous` warning already says this. Multiple sources stay
...
- **Cross-spec `sourceDescriptions`.** A scenario references one document,
  ...
  sources arrive with the composition work of `scenarios-roadmap.md` §5.3,
  and this design does not block it: the emitter copies what it was given
