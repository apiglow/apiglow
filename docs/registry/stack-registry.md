...
`tests/e2e/about.spec.js:35-44` **hard-codes** versions and must be edited
by hand. It covers six of the seven bundled credits — daisyUI, Tailwind,
ref-parser, Marked, DOMPurify, highlight.js — and **omits `json-p3`**,
which is bundled and credited like the rest; closing that gap is a
guard-net question for `app-health`, not a pin question. The hand-edit
applies to the daisyUI and Tailwind rows in the build table below as much
as to the app rows, because both are bundled credits.
...
| es-check | 9.6.4 (exact) | 9.6.4 | npm + github.com/yowainwright/es-check | enforces that target on the built bundle (`check:syntax`); pairs with the row above — bump them together when both move |
...
| JS build target | **derived, never hardcoded**: `browserslistToEsbuild()` at `vite.config.js:86` reads the `browserslist` field of `package.json` | resolves to `chrome111, edge111, firefox128, ios16.4, safari16.4` | Baseline / caniuse per feature | there is no `es20xx` literal to edit — **the floor is the knob**. `npm run check:syntax` (`es-check checkBrowser … --checkFeatures`) enforces the same list on the built bundle. Raising the floor needs every target browser per the policy **and** syntax the code actually wants — nothing wants ES2023+ today. The bake CLI is unrelated: `vite.bake.config.js` targets `node24` |
| Adoptable platform features | n/a (watch) | **0 adoptable** of the 19 that went *widely available* up to the July 2026 Baseline digest (no August digest published) | Baseline monthly digests (web.dev/blog/baseline-digest-*) | Examined and rejected, with the reason each time: `URL.canParse()` — all 7 `new URL()` try/catch sites consume the parsed object, so it would double-parse; `Array.fromAsync()` — the two `for await` sites (`src/app.js:926`, `src/scenarios/step-controller.js:122`) consume each value as it arrives and accumulate nothing; CSS `rect()` — the one clip-path (`src/styles/app.css:746-761`) insets from the edges, which is what `inset()` already says. Watch: `contrast-color()` vs the `color-contrast` waiver (`docs/architecture.md:1190`) — re-confirmed *newly* available, low_date 2026-04-10, via api.webstatus.dev/v1/features/contrast-color |
