...
| First touch: parameter prefill, blocked credential focus, environment provisioned by the first runtime write (credential, OAuth token, scenario extraction), generated onboarding page | `prefill.test.js`, `env-store.test.js` | `first-touch.spec.js`, `oauth.spec.js`, `scenarios.spec.js`, `a11y.spec.js` |
...
| Operator announcements: inline entries and the file form, levels, schedule window, dismissal memory (`docs/architecture.md` §5.17) | `announcements.test.js`, `specs.test.js` | `announcements.spec.js`, `a11y.spec.js` |
