...
import { llmsFullExporter, llmsTextExporter } from './shell/exports.js'
...
function appLayout(
  ...
  const showSetupLink = (setupPayload) => {
    ...
    if (envStore.locked) {
      showToast('error', t('envSetup.locked'))
      return
    }
    const link = decodeSetupLink(setupPayload)
    if (!link) {
      showToast('error', t('envSetup.invalid'))
      return
    }
    ...
    // environments.
    if (link.spec && link.spec !== activeSpec.id) {
      showToast('error', t('envSetup.wrongSpec', { link: link.spec, active: activeSpec.id }))
      return
    }
    const match = envStore.list().find((e) => e.name === link.env.name) ?? null
    const plan = planSetup(link, { env: match })
    setupDialog ??= layout.appendChild(document.createElement('env-setup-dialog'))
    setupDialog.onApply = () => {
  ...
  const openSetupBuilder = () => {
    if (!setupBuilder) {
      setupBuilder = layout.appendChild(document.createElement('env-setup-builder'))
      ...
      setupBuilder.specId = specsConfig.multi ? activeSpec.id : null
  ...
  const openShareDialog = (env) => {
    if (!shareDialog) {
      shareDialog = layout.appendChild(document.createElement('env-share-dialog'))
      ...
      shareDialog.specId = specsConfig.multi ? activeSpec.id : null
    }
  ...
  const overlayCount = loaded.overlays?.actions ? loaded.overlays.count : 0
  ...
  const overlayUrls = (config.openapi.overlays ?? [])
  ...
  const specDownload = {
    ...
    notes: specDownloadNotes(model, loaded.overlays),
  }
